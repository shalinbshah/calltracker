...
import com.inqbarna.tablefixheaders.adapter.TableAdapter;
...
public class TableFixHeaders extends ViewGroup {
	...
	public void scrollTo(int x, int y) {
		if (needRelayout) {
			...
			scrollBy(x - sumArray(widths, 1, firstColumn) - scrollX, y
					- sumArray(heights, 1, firstRow) - scrollY);
		}
	}
	...
	public void scrollBy(int x, int y) {
		...
		if (scrollX == 0) {
			...
			while (!rowViewList.isEmpty()
					&& getFilledWidth()
							- widths[firstColumn + rowViewList.size()] >= width) {
				removeRight();
			}
			if (rowViewList.isEmpty()) {
				while (scrollX < 0) {
		...
		if (scrollY == 0) {
			...
			while (!columnViewList.isEmpty()
					&& getFilledHeight()
							- heights[firstRow + columnViewList.size()] >= height) {
				removeBottom();
			}
			if (columnViewList.isEmpty()) {
				while (scrollY < 0) {
	...
	private int getFilledWidth() {
		return widths[0]
				+ sumArray(widths, firstColumn + 1, rowViewList.size())
				- scrollX;
	}

	private int getFilledHeight() {
		return heights[0]
				+ sumArray(heights, firstRow + 1, columnViewList.size())
				- scrollY;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (firstColumn >= columnCount
				|| getMaxScrollX() - getActualScrollX() < 0) {
			firstColumn = 0;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (needRelayout || changed) {
			...
			if (adapter != null) {
				...
				addShadow(shadows[0], widths[0], 0, widths[0] + shadowSize,
						bottom);
				addShadow(shadows[1], 0, heights[0], right, heights[0]
						+ shadowSize);
				...
				for (int i = firstColumn; i < columnCount && left < width; i++) {
					...
					final View view = makeAndSetup(-1, i, left, 0, right,
							heights[0]);
				...
				for (int i = firstRow; i < rowCount && top < height; i++) {
					...
					final View view = makeAndSetup(i, -1, 0, top, widths[0],
							bottom);
				...
				for (int i = firstRow; i < rowCount && top < height; i++) {
					...
					for (int j = firstColumn; j < columnCount && left < width; j++) {
						...
						final View view = makeAndSetup(i, j, left, top, right,
								bottom);
						list.add(view);
						left = right;
					}
	...
	private int scrollBounds(int desiredScroll, int firstCell, int sizes[],
			int viewSize) {
		if (desiredScroll == 0) {
			// no op
		} else if (desiredScroll < 0) {
			desiredScroll = Math.max(desiredScroll,
					-sumArray(sizes, 1, firstCell));
		} else {
			desiredScroll = Math.min(
					desiredScroll,
					Math.max(
							0,
							sumArray(sizes, firstCell + 1, sizes.length - 1
									- firstCell)
									+ sizes[0] - viewSize));
		}
	...
	private int[] adjustFirstCellsAndScroll(int scroll, int firstCell,
			int sizes[]) {
		if (scroll == 0) {
			// no op
		} else if (scroll > 0) {
	...
	private void shadowsVisibility() {
		...
		final int[] remainPixels = { actualScrollX, actualScrollY,
				getMaxScrollX() - actualScrollX,
				getMaxScrollY() - actualScrollY, };

		for (int i = 0; i < shadows.length; i++) {
			setAlpha(shadows[i],
					Math.min(remainPixels[i] / (float) shadowSize, 1));
		}
	}
	...
	private View makeAndSetup(int row, int column, int left, int top,
