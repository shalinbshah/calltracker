...
import com.inqbarna.tablefixheaders.adapter.BaseTableAdapter;
	...
	public View getView(int row, int column, View converView, ViewGroup parent) {
		if (converView == null) {
			converView = inflater.inflate(getLayoutResource(row, column),
					parent, false);
			if (column == -1) {
				((LinearLayout) converView).setGravity(Gravity.CENTER);
				((TextView) converView.findViewById(android.R.id.text1))
